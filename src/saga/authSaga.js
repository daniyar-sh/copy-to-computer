...
function * signUp (action) {
    ...
    try{
        ...
        yield put({typ: types.SIGN_UP_SUCCESS, payload: authResponse}) 
    }catch(error) {
...
function* signIn(action) {
    ...
    try {
        ...
        yield put({type: types.SIGN_CURRENT_USER, payload: decoded})
        if (role === 'admin') {
            navigate('/dashboard')
        }
      
    }catch(error){
        yield put({typ: types.SIGN_IN_FAILURE, error})
    }
